...
public class DataGame {
    ...
    private static final byte[] dart = FileIO.readFile("image/data/update_data/dart");
    private static final byte[] arrow = FileIO.readFile("image/data/update_data/arrow");
    private static final byte[] effect = FileIO.readFile("image/data/update_data/effect");
    private static final byte[] image = FileIO.readFile("image/data/update_data/image");
    private static final byte[] skill = FileIO.readFile("image/data/update_data/skill");
    ...
    public static void updateData(Session session) {
        //System.out.println("Tai du lieu");
        Message msg;
        try {
            msg = new Message(-87);
            msg.writer().writeByte(vsData);
    ...
    public static void updateMap(Session session) {
        Message msg;
        try {
            msg = Service.getInstance().messageNotMap((byte) 6);
            msg.writer().writeByte(vsMap);
            ...
            for (MobTemplate temp : Manager.MOB_TEMPLATES) {
                msg.writer().writeByte(temp.type);
                msg.writer().writeUTF(temp.name);
                msg.writer().writeInt(Util.GIOIHAN(temp.hp));
                msg.writer().writeByte(temp.rangeMove);
                msg.writer().writeByte(temp.speed);
                msg.writer().writeByte(temp.dartType);
            }
    ...
    public static void sendDataImageVersion(Session session) {
        ...
        try {
            msg = new Message(-111);
            msg.writer().write(
                    FileIO.readFile("image/data/data_img_version/x" + session.zoomLevel + "/img_version"));
            session.doSendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
            Log.error(DataGame.class, e);
    ...
    public static void sendDataItemBG(Session session) {
        Message msg;
        try {
            byte[] item_bg = FileIO.readFile("image/bg_data");
    ...
    public static void sendTileSetInfo(Session session) {
        ...
        try {
            msg = new Message(-82);
            msg.writer().write(FileIO.readFile("image/data/map/tile_set_info"));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
        }
    ...
    public static void sendMapTemp(Session session, int id) {
        ...
        try {
            ...
            msg.writer().write(FileIO.readFile("image/data/map/map_temp/" + id));
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
            Log.error(DataGame.class, e);
    ...
    public static void sendLinkIP(Session session) {
        ...
        try {
            ...
            //System.out.println("LINK_IP_PORT: " + LINK_IP_PORT);
            msg.writer().writeByte(2);
            session.sendMessage(msg);
            msg.cleanup();
        } catch (Exception e) {
