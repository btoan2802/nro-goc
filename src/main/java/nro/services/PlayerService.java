...
public class PlayerService {
    ...
    public void dailyLogin(Player player) {
        if (Util.compareDay(Date.from(Instant.now()), player.firstTimeLogin)) {
            player.goldChallenge = 250000000;
    ...
    public void sendTypePkToPlayer(Player player, Player rcv) {
        Message msg;
        try {
            msg = Service.getInstance().messageSubCommand((byte) 35);
            msg.writer().writeInt((int) player.id);
    ...
    public void hoiSinh(Player player) {
        ...
        // Không làm gì để tránh lỗi và tránh trừ tiền/ngọc sai
        return;
    }
        if (player.isDie()) {
            boolean canHs = false;
