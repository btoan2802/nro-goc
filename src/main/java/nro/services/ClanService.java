...
//    }
    ...
    for (Clan clan : Manager.CLANS) {
        if (clan != null && clan.name != null) {
            if (clan.name.toLowerCase().contains(search)) {
                listClan.add(clan);
                if (listClan.size() >= 20) {
                    break;
                }
    ...
    public void sendListClan(Player player, String name) {
        ...
        try {
            ...
            for (Clan clan : clans) {
                ...
                msg.writer().writeUTF(clan.slogan != null ? clan.slogan : "");
    ...
    public void RemoveClanAll(Player player) {
        Clan clan = player.clan;
        if (clan != null) {
            if (clan.banDoKhoBau == null && clan.doanhTrai == null) {
                ClanMember cm = clan.getClanMember((int) player.id);
                if (cm != null) {
                ...
                Service.getInstance().sendThongBao(player, "Hãy đợi kết thúc bản đồ kho báu hoặc doanh trại");
            }
        }
    }

    public void ExtendClan(Player player) {
        Clan clan = player.clan;
        if (clan != null) {
            ClanMember cm = clan.getClanMember((int) player.id);
            if (cm != null) {
                if (!clan.isLeader(player)) {
                    Service.getInstance().sendThongBao(player, "Phải là bang chủ mới có thể mở rộng");
                    return;
                }
                if (clan.maxMember > 20) {
                    Service.getInstance().sendThongBao(player, "Bang hội tối đa 20 thành viên");
                    return;
                }
                ...
                ClanService.gI().sendClanId(player);
                Service.getInstance().sendFlagBag(player);
                Service.getInstance().sendThongBao(player, "Bạn đã mở rộng bang thành công");
                ItemTimeService.gI().removeTextDoanhTrai(player);
            }
        }
    }
    ...
    public void leaveClan(Player player) {
        ...
        if (MapService.gI().isMapDoanhTrai(player.zone.map.mapId)
            ...
            Service.getInstance().sendThongBao(player, "Hãy thoát khỏi bản đồ kho báu hoặc doanh trại");
            return;
        }
        if (clan != null) {
    ...
    public void kickOut(Player player, int memberId) {
        ...
        if (clan != null && cm != null
            ...
            if (plKicked != null && plKicked.zone != null
                    && (MapService.gI().isMapDoanhTrai(plKicked.zone.map.mapId)
                    || MapService.gI().isMapBanDoKhoBau(plKicked.zone.map.mapId))) {
                Service.getInstance().sendThongBao(plKicked,
                        "Thành viên đang trong phó bản, không thể đuổi, hãy chờ thêm ít phút nữa");
                return;
            }
    ...
    private void checkDoneTaskJoinClan(Clan clan) {
        if (clan.getMembers().size() >= 0) { // CHECK SỐ THÀNH VIÊN TRONG BANG
