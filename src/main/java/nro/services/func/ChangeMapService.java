...
public class ChangeMapService {
    ...
    public static final byte TIME_START_SUPORT_TASK_1 = 14;
    public static final byte TIME_END_SUPORT_TASK_1 = 16;
    public static final byte TIME_START_SUPORT_TASK_2 = 18;
    public static final byte TIME_END_SUPORT_TASK_2 = 20;

    public static final byte TIME_START_FIDE_GOLD = 12;
    public static final byte TIME_END_FIDE_GOLD = 13;
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        int time = pl.itemTime.isHuyHieu ? 5000 : 10000;
        if (pl.isAdmin() || Util.canDoWithTime(pl.lastTimeChangeZone, time)) {
            ...
            Service.getInstance().sendThongBao(pl, "Không thể đổi khu vực lúc này, vui lòng đợi "
                    + TimeUtil.getTimeLeft(pl.lastTimeChangeZone, time / 1000));
    ...
    public void changeMapBySpaceShip(Player pl, int mapId, int zone, int x) {
        if (pl.isHoldNamecBall) {
            Service.getInstance().sendThongBao(pl, "Bạn đã ôm ngọc rồng Namec không thể rời khỏi vũ trụ này");
            return;
        }
        if (pl.isDie()) {
...
//            }
EscortedBoss eb = pl.getEscortedBoss();
if (eb != null) {
    // ví dụ: đổi map < 1200ms tính là bất thường
    if (!Util.canDoWithTime(pl.lastTimeChangeMap, 3000)) {
        eb.stopEscorting();
    } else {
        eb.joinMapEscort();
    }
            ...
            if (!pl.isBoss && !pl.isPet && !pl.isMiniPet) {
    ...
    // chỉ dùng cho boss
    public void changeMapBySpaceShip(Player pl, Zone zoneJoin, byte typeSpace, int x) {
        if (zoneJoin != null) {
            pl.setUseSpaceShip(typeSpace);
            pl.location.x = x;
            pl.location.y = 5;
...
//    }
    ...
    public boolean TimeHoTroNhiemVu() {
        int getHour = TimeUtil.getCurrHour();
        if ((getHour >= TIME_START_SUPORT_TASK_1 && getHour <= TIME_END_SUPORT_TASK_1)
                || (getHour >= TIME_START_SUPORT_TASK_2 && getHour <= TIME_END_SUPORT_TASK_2)) {
            return true;
        }
        return false;
    ...
    public boolean TimeBossFideGold() {
        int getHour = TimeUtil.getCurrHour();
        return getHour >= TIME_START_FIDE_GOLD && getHour <= TIME_END_FIDE_GOLD;
    }

    public String getStringHoTroNv() {
        return "Thời gian hỗ trợ nhiêm vụ từ " + TIME_START_SUPORT_TASK_1 + " giờ đến " + TIME_END_SUPORT_TASK_1
                + " giờ và " + TIME_START_SUPORT_TASK_2 + " đến " + TIME_END_SUPORT_TASK_2 + " giờ hằng ngày";
    }

    private Zone notifyAndReturn(Player player) {
        Service.getInstance().sendThongBao(player,
                "Đang trong giờ chan (Hỗ trợ nhiệm vụ), không thể vào khu");
        return null;
    }
    ...
    public Zone checkMapCanJoin(Player player, Zone zoneJoin) {
        ...
        if (TimeUtil.getCurrHour() % 2 == 0 && zoneJoin != null) {
            for (Player b : zoneJoin.getBosses()) {
                int getTaskPlayer = TaskService.gI().getIdTask(player);

                if (b.id == BossFactory.KUKU && getTaskPlayer != ConstTask.TASK_19_0) {
                    notifyAndReturn(player);
                }

                if (b.id == BossFactory.MAP_DAU_DINH && getTaskPlayer != ConstTask.TASK_19_1) {
                    notifyAndReturn(player);
                }

                if (b.id == BossFactory.RAMBO && getTaskPlayer != ConstTask.TASK_19_2) {
                    notifyAndReturn(player);
                }

                if ((b.id == BossFactory.TIEU_DOI_TRUONG || b.id == BossFactory.SO1
                        || b.id == BossFactory.SO2 || b.id == BossFactory.SO3 || b.id == BossFactory.SO4)
                        && (getTaskPlayer < ConstTask.TASK_20_1 || getTaskPlayer > ConstTask.TASK_20_5)) {
                    notifyAndReturn(player);
                }

                if ((b.id == BossFactory.FIDE_DAI_CA_1 || b.id == BossFactory.FIDE_DAI_CA_2
                        || b.id == BossFactory.FIDE_DAI_CA_3)
                        && (getTaskPlayer < ConstTask.TASK_21_1 || getTaskPlayer > ConstTask.TASK_21_3)) {
                    notifyAndReturn(player);
                }

                if ((b.id == BossFactory.ANDROID_19 || b.id == BossFactory.ANDROID_20)
                        && (getTaskPlayer < ConstTask.TASK_22_1 || getTaskPlayer > ConstTask.TASK_22_3)) {
                    notifyAndReturn(player);
                }

                if ((b.id == BossFactory.ANDROID_13 || b.id == BossFactory.ANDROID_14
                        || b.id == BossFactory.ANDROID_15)
                        && (getTaskPlayer < ConstTask.TASK_23_1 || getTaskPlayer > ConstTask.TASK_23_3)) {
                    notifyAndReturn(player);
                }

                if ((b.id == BossFactory.PIC || b.id == BossFactory.POC || b.id == BossFactory.KINGKONG)
                        && (getTaskPlayer < ConstTask.TASK_24_1 || getTaskPlayer > ConstTask.TASK_24_3)) {
                    notifyAndReturn(player);
                }

                if ((b.id == BossFactory.XEN_BO_HUNG_1 || b.id == BossFactory.XEN_BO_HUNG_2
                        || b.id == BossFactory.XEN_BO_HUNG_HOAN_THIEN)
                        && (getTaskPlayer < ConstTask.TASK_25_1 || getTaskPlayer > ConstTask.TASK_25_3)) {
                    notifyAndReturn(player);
                }
            }
        }
...
//                        Service.getInstance().sendThongBao(player, "Hãy hoàn thành nhiệm vụ trước");
//                        return null;
//                    }
//                    break;
                case 5: // đảo kame
...
//                    }
    ...
    public boolean isTaskSupport(int idTask) {
        ...
        if (player != null && player.zone != null) {
            switch (player.zone.map.mapId) {
                ...
                case 91:
                    ...
                    break;
                case 188:
                    DaichienWar.gI().joinMapDaichien(player);
                    break;
                case 114:
