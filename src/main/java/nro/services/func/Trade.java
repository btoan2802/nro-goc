...
public class Trade {
    ...
    public void addItemTrade(Player pl, byte index, int quantity) {
        ...
        if (pl.getSession().actived) {
            ...
            Service.getInstance().sendThongBaoFromAdmin(pl,
                    "|5|VUI LÒNG NẠP ĐẦU HOẶC QUA NV TDST\n|7|" + Manager.DOMAIN
    ...
    private void startTrade() {
        ...
        if (!player1.getSession().actived || !player2.getSession().actived) {
            tradeStatus = FAIL_ACTIVE;
        }
        if (goldTrade2 > 5000000 || goldTrade1 > 5000000) {
            tradeStatus = FAIL_MAX_GOLD_LIMIT;
        }
        if (tradeStatus != SUCCESS) {
            ...
            // player 2 nhận
            int valueGoldBar_1 = 0;
            int valueGoldBar_2 = 0;

            List<Item> itemsTrade1_cache = new ArrayList<>();
            List<Item> itemsTrade2_cache = new ArrayList<>();

            for (Item item : itemsTrade1) {
                if (item.template.id == 457) {
                    valueGoldBar_1 += item.quantity;
                }
                Item itemTrade = ItemService.gI().copyItem(item);
                itemsTrade1_cache.add(itemTrade);
            ...
            if (tradeStatus != SUCCESS) {
                ...
                for (Item item : itemsTrade2) {
                    if (item.template.id == 457) {
                        valueGoldBar_2 += item.quantity;
                    }
                    Item itemTrade = ItemService.gI().copyItem(item);
                    itemsTrade2_cache.add(itemTrade);
                ...
                if (tradeStatus == SUCCESS) {
                    ...
                    HistoryTransactionDAO.insert(player1, player2, goldTrade1, goldTrade2, itemsTrade1_cache,
                            itemsTrade2_cache,
                    ...
                    if (valueGoldBar_1 > 0) {
                        Logger.errorSaveHistGoldBar(player2, valueGoldBar_1, (byte) 2, "Giao dịch với " + player1.name);
                    }
                    if (valueGoldBar_2 > 0) {
                        Logger.errorSaveHistGoldBar(player1, valueGoldBar_2, (byte) 2, "Giao dịch với " + player2.name);
                    }
    ...
    private static final byte FAIL_MAX_GOLD_LIMIT = 5;
    private static final byte FAIL_ACTIVE = 6;
    ...
    private void sendNotifyTrade(byte status) {
        switch (status) {
            ...
            case FAIL_NOT_ENOUGH_BAG_P2:
                ...
                break;
            case FAIL_MAX_GOLD_LIMIT:
                Service.getInstance().sendThongBao(player1,
                        "Giao dịch tối đa 50 triệu vàng");
                Service.getInstance().sendThongBao(player2,
                        "Giao dịch tối đa 50 triệu vàng");
                break;
            case FAIL_ACTIVE:
                Service.getInstance().sendThongBao(player1,
                        "Yêu cầu cả 2 phải mở thành viên");
                Service.getInstance().sendThongBao(player2,
                        "Yêu cầu cả 2 phải mở thành viên");
                break;
        }
