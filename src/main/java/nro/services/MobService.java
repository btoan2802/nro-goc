...
public class MobService {
    ...
    public void hutItem(Player player, List<ItemMap> items) {
        if (!player.isPet) {
            for (ItemMap item : items) {
                if (item.itemTemplate.id == 861) {
                    ItemMapService.gI().pickItem(player, item.itemMapId, true);
                } else if (item.itemTemplate.id != 590) {
                    if (MapService.gI().isMapPk(player.zone.map) || player.charms.tdThuHut > System.currentTimeMillis()) {
                        ItemMapService.gI().pickItem(player, item.itemMapId, true);
                    }
                }
            }
            ...
            for (ItemMap item : items) {
                if (item.itemTemplate.id == 861) {
                    ...
                    if (MapService.gI().isMapPk(player.zone.map) || ((Pet) player).master.charms.tdThuHut > System.currentTimeMillis()) {
                        ItemMapService.gI().pickItem(((Pet) player).master, item.itemMapId, true);
                    }
                }
            }
    ...
    public int mobAttackPlayer(Mob mob, Player player) {
        ...
        if (player.itemTime != null && player.itemTime.isUseGroup_4_1) {
            dameMob /= 10;
        }
        if (player.isPet) {
            if (((Pet) player).master.itemTime != null) {
                if (((Pet) player).master.itemTime.isUseGroup_4_1) {
                    dameMob /= 10;
                }
            }
        }
    ...
    public void initMobDoanhTrai(Mob mob, Clan clan) {
        ...
        long maxDameMob = mob.point.clanMemHighestHp / mob.point.xHpForDame;
        if (maxDameMob > 500000000) {
            maxDameMob = 500000000;
        }
        mob.point.dame = (int) maxDameMob;
        for (ClanMember cm : clan.getMembers()) {
            for (Player pl : clan.membersInGame) {
        ...
        long maxHpMob = mob.point.clanMemHighestDame * mob.point.xDameForHp;
        if (maxHpMob > 2000000000) {
            maxHpMob = 2000000000;
        }
        mob.point.maxHp = (int) maxHpMob;
    }

    public void initMobDoanhTrai(Mob mob, long point) {
        mob.point.hp = mob.point.maxHp;
        mob.point.dame = mob.point.dame;
    }
    ...
    public boolean isMonterFly(int tempId) {
        return tempId == ConstMob.QUY_BAY || tempId == ConstMob.QUY_BAY_ME || tempId == ConstMob.QUY_BAY_2
                || tempId == ConstMob.THAN_LAN_BAY || tempId == ConstMob.THAN_LAN_BAY_2;
    }
}
