    ...
    public void goToPlayerWithYardrat(Player player, Message msg) {
        try {
            ...
            if (pl != null && pl.zone != null) {
                ...
                if (player.isAdmin() || player.nPoint.teleport) {
                    ...
                    if (!pl.itemTime.isUseAnDanh || player.isAdmin()) {
                        if (player.isAdmin() || !pl.zone.isFullPlayer() || !(mapid >= 85 && mapid <= 91
                            ...
                            if ((mapGo = ChangeMapService.gI().checkMapCanJoin(player, mapGo)) != null
                                    && !mapGo.isFullPlayer() && !MapService.gI().isMapBH(mapid)) {
                                ChangeMapService.gI().changeMapYardrat(player, pl.zone,
                ...
                Service.getInstance().sendThongBao(player, "Người chơi đã offline");
            }
        } catch (IOException ex) {
