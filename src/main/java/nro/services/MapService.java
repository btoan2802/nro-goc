...
public class MapService {
    ...
    public Zone getMapCanJoin(Player player, int mapId) {
        ...
        for (Zone zone : map.zones) {
            if (MapService.gI().isMapBlackBallWar(map.mapId)) {
                if (zone.getNumOfPlayers() < 14) {
                    mapJoin = zone;
                    break;
                }
            } else if (zone.getNumOfPlayers() < Zone.PLAYERS_TIEU_CHUAN_TRONG_MAP) {
                mapJoin = zone;
                break;
            }
        }
        if (mapJoin == null) {
            for (Zone zone : map.zones) {
                if (zone.getNumOfPlayers() < zone.maxPlayer - 1) {
                    mapJoin = zone;
                    break;
                }
            }
    ...
    public Zone getMapWithRandZone(int mapId) {
        ...
        try {
            if (map != null) {
                zone = map.zones.get(Util.nextInt(2, map.zones.size() - 1));
            }
        } catch (Exception e) {
        }
        return zone;
    ...
    public List<Zone> getMapCapsule(Player pl) {
        ...
        if (pl.mapBeforeCapsule != null
                ...
                && !MapService.gI().isMapBH(pl.mapBeforeCapsule.map.mapId)) {
    ...
    public boolean isLanhDia(int mapId) {
        return mapId >= 153 && mapId <= 159;
    }

    public boolean isMapthientu(Map map) {
        int mapId = map.mapId;
        return mapId == 185 || mapId == 186;
    }
    
    public boolean isMapSKtt(Map map) {
        int mapId = map.mapId;
        return mapId == 189 || mapId == 191 || mapId == 190;
    }

    public boolean isMaptv(Map map) {
        int mapId = map.mapId;
        return mapId == 179 || mapId == 180 || mapId == 181;
    }

    public boolean isMapPk(Map map) {
        int mapId = map.mapId;
        return mapId == 188;
    }
    ...
    public boolean isMapHtnt(int mapId) {
        return mapId == 155;
    }
    ...
    public boolean isVegetable(int mapId) {
    ...
    public boolean isMapBanDoKhoBau_new(int mapId) {
        return mapId >= 164 && mapId <= 167;
    }

    public boolean isMapHacTinh(int mapId) {
        return mapId >= 139 && mapId <= 140;
    }

    public boolean isMapBH(int mapId) {
        return mapId == 188;
    }

    public boolean isMapContrung(int mapId) {
        return mapId >= 168 && mapId <= 170;
    }

    public boolean isMapDaiHaiTrinh(int mapId) {
        return mapId >= 179 && mapId <= 181;
    }

    public boolean isMapCereal(int mapId) {
        return mapId >= 196 && mapId <= 198;
    }

    public boolean isMapRungNguyenSinh177(int mapID) {
        return mapID == 177;
    }
}
