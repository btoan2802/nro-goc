...
public class EffSkinService {
    ...
    public void setBiNgo(Player player, long lastTimeBiNgo, int timeBiNgo) {
        player.effectSkin.lastTimeBiNgo = lastTimeBiNgo;
        player.effectSkin.timeBiNgo = timeBiNgo;
        player.effectSkin.isBiNgo = true;
        Service.getInstance().point(player);
    }

    public void removeBiNgo(Player player) {
        player.effectSkin.isBiNgo = false;
        Service.getInstance().point(player);
        Service.getInstance().Send_Caitrang(player);
    }

    public void setCarrot(Player player) {
        if (player.effectSkin.isCarrot = false) {
            player.effectSkin.lastTimesCarrot = System.currentTimeMillis();
            player.effectSkin.timeCarrot = 300000;
    ...
    public void removeCarrot(Player player) {
        player.effectSkin.isCarrot = false;
        Service.getInstance().point(player);
        Service.getInstance().Send_Caitrang(player);
