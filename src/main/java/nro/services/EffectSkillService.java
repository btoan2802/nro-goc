...
import nro.models.boss.tieudoiBoJack.S_Bojack;
...
public class EffectSkillService {
    ...
    public void setIsMonkey(Player player) {
        ...
        // Log.warning("Hp 1" + player.nPoint.hp);
        long setHp = player.nPoint.calLimit((long) player.nPoint.hp * 2);

        int setDame = player.nPoint.calLimit((long) player.nPoint.dame * 2);

        player.nPoint.setHp((int) setHp);
        player.nPoint.setDame(setDame);
    }
    ...
    public void stopDoc(Player player) {
        player.effectSkill.countDoc = 0;
        player.effectSkill.isAnDoc = false;

    }

    public void startDoc(Player player) {
        player.effectSkill.countDoc = 0;
        player.effectSkill.isAnDoc = true;
        ItemTimeService.gI().sendItemTime(player, 10713, 10);
    }
    ...
    public void sendEffectBlindThaiDuongHaSan(Player plUseSkill, List<Player> players, List<Mob> mobs, int timeStun) {
        ...
        try {
            ...
            for (Player pl : players) {
                ...
                msg.writer().writeByte((timeStun - timeGiamChoang) / 1000);
            }
            Service.getInstance().sendMessAllPlayerInMap(plUseSkill, msg);
            msg.cleanup();
        } catch (Exception e) {
    ...
    public void setIsSaiYan(Player player) {
        ...
        player.effectSkill.levelBienHinh = (byte) point;
        ...
        player.effectSkill.levelSaiYan = (byte) 1;
        RadaService.getInstance().setIDAuraEff(player, (byte) getIdaura(player.gender, 1));
        Service.getInstance().point(player);
    }
