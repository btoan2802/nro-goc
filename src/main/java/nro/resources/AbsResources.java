...
//        initEffectData();
    ...
    public void initBGSmallVersion() {
        try {
            ...
            for (int i = 0; i < 4; i++) {
                File file = new File(this.folder, (i + 1) + "/bg/");
    ...
    public void initSmallVersion() {
        try {
            ...
            for (int i = 0; i < 4; i++) {
                File file = new File(this.folder, (i + 1) + "/icon/");
                File[] files = file.listFiles();
                int max = 0;
                for (File f : files) {
                    String name = f.getName();
    ...
    protected void setFolder(String folder) {
        this.folder = new File("image", folder);
    }
    ...
    public byte[] getRawIconData(int zoomLevel, int iconID) {
        return readAllBytes(zoomLevel + "/icon/" + iconID + ".png");
    }

    public byte[] getRawBGData(int zoomLevel, int bg) {
        return readAllBytes(zoomLevel + "/bg/" + bg + ".png");
    }

    public byte[] getRawIBNData(int zoomLevel, String filename) {
        return readAllBytes(zoomLevel + "/imgbyname/" + filename + ".png");
    }

    public byte[] getRawMobData(int zoomLevel, int id) {
        return readAllBytes(zoomLevel + "/monster/" + id + ".png");
    }

    public byte[] getRawEffectData(int zoomLevel, int id) {
        return readAllBytes(zoomLevel + "/effect/" + id + ".png");
    }
