...
import nro.jdbc.daos.LuckyDAO;
import nro.jdbc.daos.RatioDAO;
...
public final class Manager {
    ...
    public static String DOMAIN = "ngocronghero.com";
    public static String SERVER_NAME = "NroHero";
    ...
    public static int apiPort = 8180;
    public static int bossGroup = 5;
    public static int workerGroup = 10;
    public static String apiKey = "hjakdja";
    public static String executeCommand;
    public static boolean debug;
    public static int EVENT_POINT_TET_2024 = 0;
    public static long TOP_DAME;
    public static final List<String> TOP_PLAYERS = new ArrayList<>();
    public static int RATIO_UP_DO_THAN = 85000;
    public static int RATIO_ROI_DO_THAN = 2;
    ...
    public static final List<ItemLuckyRound> LUCKY_SHOP = new ArrayList<>();
    public static final List<Item> LIST_ITEM_LUCKY_REWARD = new ArrayList<>();
    public static final RandomCollection<ItemLuckyRound> MAY_GAP_THU_THUONG = new RandomCollection<>();
    public static final RandomCollection<ItemLuckyRound> MAY_GAP_THU_CAO_CAP = new RandomCollection<>();
    public static final RandomCollection<ItemLuckyRound> MAY_GAP_THU_VIP = new RandomCollection<>();
    public static final List<Item.ArrHead2Frames> ARR_HEAD_2_FRAMES = new ArrayList<>();
    ...
    public static boolean is_reload_shop;
    public static boolean is_reload_boss;
    public static boolean cheat;
    public static int x, y, z;
    public static long lastTimeADdTotalgoldBaucua;
    public static boolean BotBauCua;
    @Getter
    ...
    private void initRandomItem() {
        ...
        HOP_QUA_TET.add(10, ConstItem.VE_DOI_AO_HUY_DIET);
        HOP_QUA_TET.add(10, ConstItem.VE_DOI_QUAN_HUY_DIET);
        HOP_QUA_TET.add(10, ConstItem.VE_DOI_GIAY_HUY_DIET);
        HOP_QUA_TET.add(10, ConstItem.VE_DOI_NHAN_HUY_DIET);
        HOP_QUA_TET.add(10, ConstItem.VE_DOI_GANG_HUY_DIET);
    ...
    private void initMap() {
        ...
        for (MapTemplate mapTemp : MAP_TEMPLATES) {
            ...
            if (mapTemp.id == 126) {
                ...
                // santaCity.timer(18, 0, 0, 3600000);
            } else {
        ...
        try (Connection con = DBService.gI().getConnectionForGame();) {
            ...
            if (rs.first()) {
                ...
                while (rs.next()) {
                    ...
                    dataArray = (JSONArray) jv.parse(rs.getString("effect_noel"));
                    for (int j = 0; j < dataArray.size(); j++) {
                        EffectMap em = new EffectMap();
                        dataObject = (JSONObject) jv.parse(dataArray.get(j).toString());
                        String get_value = String.valueOf(dataObject.get("value"));
                        em.setKey(String.valueOf(dataObject.get("key")));
                        em.setValue(get_value);
                        if (Manager.EVENT_SEVER != 4 && get_value.length() >= 2) {
                            if (get_value.substring(0, 2).equals("76") || get_value.substring(0, 2).equals("77")) {
                                continue;
                            }
                        }
...
//                    dataObject.clear();
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                Item.ArrHead2Frames arrHead2Frames = new Item.ArrHead2Frames();
            ...
            Log.success("Load head 2 frames thanh cong (" + ARR_HEAD_2_FRAMES.size() + ")");
            ...
            rs.close();
            ps.close();
            Log.success("Load map hú búuuuuuuuuuuuuuu item template thành công (" + ITEM_TEMPLATES.size() + ")");
            ...
            LuckyDAO.gI().LoadItemLucky(con);
            Log.success("Load reward lucky round thành công! ");
            // load tỉ lệ
            RatioDAO.gI().Load(con);
            Log.success("Load ratio round thành công! ");
            // load máy gắp thú thường
            File folder = new File("image/data/data_may_gap_thu_thuong");
            ...
            // load máy gắp thú cao cấp
            folder = new File("image/data/data_may_gap_thu_cao_cap");
            for (File fileEntry : folder.listFiles()) {
                if (!fileEntry.isDirectory()) {
                    BufferedReader br = new BufferedReader(new FileReader(fileEntry));
                    String line = null;
                    while ((line = br.readLine()) != null) {
                        ...
                        MAY_GAP_THU_CAO_CAP.add(item.ratio, item);

                    }
                    Log.success("Load reward gắp thú cao cấp thành công! ");
                }
            }
            // load máy gắp thú vip
            folder = new File("image/data/data_may_gap_thu_vip");
            for (File fileEntry : folder.listFiles()) {
                if (!fileEntry.isDirectory()) {
                    BufferedReader br = new BufferedReader(new FileReader(fileEntry));
                    String line = null;
                    while ((line = br.readLine()) != null) {
                        ...
                        MAY_GAP_THU_VIP.add(item.ratio, item);
            ...
            // load reward mob
            folder = new File("image/data/data_mob_reward");
            for (File fileEntry : folder.listFiles()) {
                if (!fileEntry.isDirectory()) {
                    BufferedReader br = new BufferedReader(new FileReader(fileEntry));
                    String line = null;
...
//        String linkServer = SettingGame.NAME_GAME + ":" + SettingGame.LinkIp + ":" + ServerManager.PORT + ":0";
//        DataGame.LINK_IP_PORT = linkServer;
        Log.warning("LINK_IP_PORT " + DataGame.LINK_IP_PORT);
        SERVER_NAME = SettingGame.NAME_GAME;
        DOMAIN = SettingGame.LINK_GAME;
        ServerManager.NAME = SettingGame.NAME_GAME;
    ...
    private int[][] readTileIndexTileType(int tileTypeFocus) {
        ...
        try {
            DataInputStream dis = new DataInputStream(new FileInputStream("image/data/map/tile_set_info"));
    ...
    private int[][] readTileMap(int mapId) {
        ...
        try {
            ...
            // return tileMap;
            // }
            DataInputStream dis = new DataInputStream(new FileInputStream("image/data/map/map_temp/" + mapId));
            int w = dis.readByte();
            int h = dis.readByte();
            tileMap = new int[h][w];
    ...
    public void ReloadRatio() {
        try (Connection con = DBService.gI().getConnectionForLoadData()) {
    ...
    public void ReloadBoss() {
        List<Boss> bossRemove = new ArrayList<>();
        is_reload_boss = true;
        try {
            ...
            Log.error(Manager.class, e, "Lỗi reset boss");
        } finally {
            is_reload_boss = false;
        }
    }

    public static void ReloadShop() {
        // Đặt is_reload_shop thành true để báo hiệu đang tiến hành tải lại dữ liệu
        is_reload_shop = true;

        // Khởi tạo connection và thực hiện các thao tác tải dữ liệu
        try (Connection con = DBService.gI().getConnectionForLoadData()) {
            // Xóa hết các dữ liệu cũ trong SHOPS trước khi tải lại
            SHOPS.clear();

            // Tải danh sách các cửa hàng
            List<Shop> shops = ShopDAO.getShops(con);
            SHOPS.addAll(shops); // Thêm danh sách các cửa hàng vào SHOPS

            // Tải lại các mục may mắn (lucky items)
            LuckyDAO.gI().LoadItemLucky(con);
        } catch (Exception e) {
            Log.error(Manager.class, e, "Lỗi load database");
        } finally {
            // Đặt is_reload_shop thành false khi hoàn thành việc tải lại dữ liệu, bao gồm
            is_reload_shop = false;
        }
    }
