...
public class ConsignManager {
    ...
    public void load() {
        try {
            ...
            while (rs.next()) {
                ...
                ConsignmentItem item = ItemService.gI().createNewConsignmentItem(itemID,
                        quantity);
                ...
                item.setIdOrder(rs.getInt("order"));
    ...
    public void save() {
        try (Connection con = DBService.gI().getConnectionForKyGui(); PreparedStatement truncateStatement = con.prepareStatement("TRUNCATE consignment_shop"); PreparedStatement insertStatement = con.prepareStatement(
                "INSERT INTO `consignment_shop`(`id`, `consignor_id`, `tab`, `item_id`, `gold`, `gem`,"
                + "`order`,`quantity`, `item_options`, `up_top`, `sold`,`time_consign`) VALUES (? ,?, ?,"
                + "?, ?, ?, ?, ?, ?, ?, ?,?)")) {
            ...
            for (ConsignmentItem it : list) {
                if (it != null) {
                    ...
                    insertStatement.setInt(7, it.getIdOrder());
                    insertStatement.setInt(8, it.quantity);
                    insertStatement.setString(9, options.toJSONString());
                    insertStatement.setBoolean(10, it.isUpTop());
                    insertStatement.setBoolean(11, it.isSold());
                    insertStatement.setLong(12, it.createTime);
