...
public class NamekBallWar {
    ...
    public void pickBall(Player player, NamekBall item) {
        ...
        Service.getInstance().sendThongBao(player,
                "Bạn đang giữ viên ngọc rồng Namek " + (item.getIndex() + 1) + " sao");
    }

    public void summonDragon(Player player, Npc npc) {
        Clan clan = player.clan;
        if (clan == null) {
            npc.npcChat(player, "Hãy thành lập bang hội trước");
            return;
        }
        int clanID = clan.id;
        if (holders[0] != player) {
            npc.npcChat(player, "Hãy đem viên ngọc 1 sao tới gặp em để có thể triệu hồi rồng thần");
            return;
        }
        ...
        if (!oneStar.isCleaning()) {
            ...
            npc.npcChat(player,
                    "Hãy đợi 9 phút em lau sạch ngọc rồng,trong thời gian ngọc hồi phục nếu một trong các viên ngọc bị rơi sẽ làm mới thời gian lau");
            return;
        }
        if (oneStar.isCleaning() && oneStar.getCleaningTime() == 0) {
            ...
            npc.npcChat(player,
                    "Viên ngọc rồng đang được lau sạch, còn " + oneStar.getCleaningTime() + " giây nữa");
        }
    }
