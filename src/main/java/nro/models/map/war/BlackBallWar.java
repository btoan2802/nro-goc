...
public class BlackBallWar {
    ...
    private void win(Player player) {
        ...
        Logger.errorSaveSQL(player.name + " win " + star + " sao");
            ...
            try {
                ...
                for (Player pl : playerGive) {
                    ...
                    Service.getInstance().sendThongBao(pl, "Chúc mừng bang hội của bạn đã "
                            + "dành chiến thắng ngọc rồng sao đen " + star + " sao");
                    Logger.error(pl.name + " da nhan thuong nro den " + star + " sao");
                }
            } catch (Exception e) {
    ...
    public void xHPKI(Player player, byte x) {
        ...
        if (player.inventory.gold >= cost) {
            ...
            long hpBase = (long) player.nPoint.hp * x;
            long mpBase = (long) player.nPoint.mp * x;
            player.nPoint.setHp(hpBase);
            player.nPoint.setMp(mpBase);
