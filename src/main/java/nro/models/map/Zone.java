...
public class Zone {
    ...
    synchronized (notBosses) {
        snapshot = new ArrayList<>(notBosses);
    }
    ...
    private void updateItem() {
        synchronized (items) {
            try {
                ...
                Logger.logException(Map.class, e, "Lỗi update item " + this.zoneId);
            }
        }
    }
    ...
    public void update() {
        try {
            ...
            Logger.logException(Zone.class, e, "Lỗi updateMob trong zone " + this.zoneId);
            e.printStackTrace();
        }
        ...
        try {
            ...
            Logger.logException(Zone.class, e, "Lỗi updatePlayer trong zone " + this.zoneId);
            e.printStackTrace();
        }
        ...
        try {
            ...
            Logger.logException(Zone.class, e, "Lỗi updateItem trong zone " + this.zoneId);
            e.printStackTrace();
        }
        ...
        if (map.mapId == ConstMap.DAI_HOI_VO_THUAT) {
            try {
                ...
                Logger.logException(Zone.class, e, "Lỗi updateReferee trong zone " + this.zoneId);
                e.printStackTrace();
            }
        }
    }
    ...
    // }
    // } else {
    // Service.getInstance().sendThongBao(player, "Không thể nhặt vật phẩm của người
    // khác");
    // }
    // }
    // }
...
//            }
//        } catch (Exception e) {
//            Log.error(Zone.class, e, "Lỗi map player " + player.name);
//        }
//    }

    public void loadAnotherToMe(Player player) { // load những player trong map và gửi cho player vào map
    try {
        // Chụp snapshot để tránh ConcurrentModificationException
        final java.util.ArrayList<Player> snapshot;
        synchronized (this.humanoids) {
            snapshot = new java.util.ArrayList<>(this.humanoids);
        }
    ...
    private void infoPlayer(Player plReceive, Player plInfo) {
        ...
        try {
            ...
            Log.error(Zone.class, e, "Loi send map zone");
        } finally {
            if (msg != null) {
                msg.cleanup();
            }
        ...
        if (!plInfo.isBoss && !plInfo.isPet && !plInfo.isMiniPet) {
            if (plInfo.inventory.itemsBody.size() > 11 && plInfo.inventory.itemsBody.get(11).isNotNullItem()) {
                SendEffect.getInstance().sendChanThienTuAll(plInfo, plReceive, (short) plInfo.inventory.itemsBody.get(11).template.id);
            }
        }
        if (!plInfo.isBoss && !plInfo.isPet && !plInfo.isMiniPet) {
            if (plInfo.inventory.itemsBody.size() > 12 && plInfo.inventory.itemsBody.get(12).isNotNullItem()) {
                SendEffect.getInstance().sendDanhhieuAll(plInfo, plReceive, (short) plInfo.inventory.itemsBody.get(12).template.id);
            }
        }
            if (!plInfo.isBoss && !plInfo.isPet && !plInfo.isMiniPet) {
                Service.getInstance().get_all_title(plReceive, plInfo);
            }
        try {
            if (plInfo.isDie()) {
                msg = new Message(-8);
    ...
    public void mapInfo(Player pl) {
        ...
        try {
            ...
            // msg.writer().writeShort(0);
            try {
                byte[] bgItem = FileIO.readFile("image/data/map/item_bg_map_data/" + this.map.mapId);
    ...
    public Player findPlayerByMobAttack(long id) {
        if (id < 0) {
            return null;
        }
        for (Player p : this.players) {
    ...
    public Player getPlayerSaveSkill(long id) {
        for (Player p : this.humanoids) {
            if (p != null && p.id == id && !p.isDie()) {
                return p;
            }
        }
        return null;
