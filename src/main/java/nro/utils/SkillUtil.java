...
public class SkillUtil {
    ...
    public static int getTimeSurviveMobMe(int level, boolean is_set_pikodaimao) { // thời gian trứng tồn tại
        int time = getTimeMonkey(level) * 2;
        if (is_set_pikodaimao) {
            time *= 2;
        }
        return time;
    }
    ...
    public static byte getTempSkillSkillByItemID(int id) {
        ...
        } else if (id >= 1446 && id <= 1460) {
            return Skill.SAYIAN_HOA;
        } else if (id >= 1461 && id <= 1475) {
            return Skill.NAP_NANG_LUONG;
        } else {
            return -1;
        }
    }
    ...
    public static Skill getSkillByItemID(Player pl, int tempId) {
        ...
        } else if (tempId >= 1446 && tempId <= 1460) {
            return getSkillbyId(pl, Skill.SAYIAN_HOA);
        } else if (tempId >= 1461 && tempId <= 1475) {
            return getSkillbyId(pl, Skill.NAP_NANG_LUONG);
        } else {
            return null;
        }
    }
